...
public class Code04_RestoreWays {
    ...
    public static int process4(int[] arr, int i, int pre, int status) {
        int N = arr.length;
        if (i == N-1) return ((status!=2&&arr[i]<=pre)||(status==2&&(arr[i]==pre||arr[i]==0)))?1:0;
        int ways = 0;
        if (arr[i] == 0) {
            for (int change = 1; change <= 200; change++) {
                if (status == 0 || status == 1) ways += process4(arr, i + 1, change, status(arr, change, i - 1));
                else if (change>=pre) ways += process4(arr, i + 1, change, status(arr, change, i - 1));
            }
        } else {
            if (status==0||status==1||arr[i]>=pre) ways+=process4(arr,i+1,arr[i],status(arr,arr[i],i-1));
        }
        return ways;
    }

    public static int status(int[] arr, int pre, int index) {// [index] pre [i]
        if (arr[index]>pre) return 0;
        if (arr[index]==pre) return 1;
        return 2;
    }
